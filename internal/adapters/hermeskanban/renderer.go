...
// worker at run time and stays a placeholder here. Since work-receipt/v2
// (E12-T3, FBK-009) the four outcomes are named with their required
// members: completed carries the manifest of changed paths,
// partially_completed adds the remaining scope, blocked reports the
// manual reason, and failed keeps the closed failure-code set.
...
...
agent-dispatch work complete --dispatch-id %s --run-id <run-id> --status completed --manifest <file>
agent-dispatch work complete --dispatch-id %s --run-id <run-id> --status partially_completed --manifest <file> --remaining-manifest <file>
agent-dispatch work complete --dispatch-id %s --run-id <run-id> --status blocked --manual-reason <bounded text>
agent-dispatch work fail --dispatch-id %s --run-id <run-id> --failure-code <agent_error|canceled|timeout|environment_error>`, req.DispatchID, req.DispatchID, req.DispatchID, req.DispatchID, req.DispatchID)
}
