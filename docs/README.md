...
7. Examples
8. Original source inputs under [`docs/99-source/`](docs/99-source/)

Source inputs are retained for provenance. They are not authoritative where this SOT resolves or changes a decision.
...
| v0.1.5 operational loop | [`multi-destination-operational-loop.md`](docs/01-architecture/multi-destination-operational-loop.md) |
...
## Approved v0.1.5 Target Shape

```text
configured Watchman subtree
    -> fenced aggregate event
        -> one independent child per selected destination/workstream
            -> Hermes public Kanban interface
                -> bounded worker receipt
        -> aggregate status and durable notification outbox
```

D-025 approves this target without changing Hermes. The full design is in
[`multi-destination-operational-loop.md`](docs/01-architecture/multi-destination-operational-loop.md);
E10 through E13 are Planned and gates G6 through G9 have no executable evidence
yet. The checked-in schemas, examples, and packaged worker skill continue to
describe v0.1.4 until their owning roadmap tasks implement the new contracts.
