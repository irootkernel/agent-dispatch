...
| AC-403 an exactly matching receipt may suppress and the decision is audited | `TestG4FeedbackLoopGate` (the generation-3 exact receipt clears the route to IDLE with both paths verified and the `attribution` audit transition carrying `verified_self_generated`) and `TestExactMatchSuppressesSelfChange` |
| AC-404 missing paths, extra paths, or a mismatched digest never suppress | `TestG4FeedbackLoopGate` (a tampered digest keeps the route on the bounded follow-up path) and `TestMismatchStaysDirty` plus the full validation table in `TestWorkCompleteValidatesManifest` |
