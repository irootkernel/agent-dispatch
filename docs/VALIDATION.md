...
| AC-1303 two release builds byte-identical, versioned documentation and skills agree | Two consecutive `make release VERSION=v0.1.6` builds from the final implementation tree (clean worktree at commit `24e07c4`, pinned Go 1.26.6, carrying every whole-epic audit remediation) produce the byte-identical darwin/arm64 binary — `SHA256SUMS` records `ea0eb3cf17820baa2d9ce76b8a5539dca6f3c3130e5501f7b2bba08dd702deae` and `cmp` proves the artifacts identical; only documentation commits follow that tree, and the publication step re-verifies byte-identity from the tagged release commit — and the versioned surfaces agree on v0.1.6: the binary's `version` output (v0.1.6, schema range 1-20), the operator skill 2.1.0's compatibility declaration, the v0.1.6 release notes, the synchronized status documents, and `TestG5AC506ReleaseArtifactsPresent` re-verifying every `dist/SHA256SUMS` digest against the hashed artifact when `dist/` is present |
