...
Verified 2026-08-20 by executable acceptance tests in `internal/app/dispatch/g2_test.go`, driving real SQLite files with real process deaths through the `internal/testsupport/crashbin` harness (TST-002: real files, never mocks, for SQLite behavior; TST-005: multiple one-shot processes against one database). Crash-boundary scope per the 2026-08-22 compliance review (TST-004): process deaths cover the before-commit and after-commit boundaries and the migration-unit edge; the during-submit and after-remote-acceptance boundaries are currently exercised only in-process, and AC-203/AC-207 rows below carry their corrections; full process-death coverage of every defined boundary is restored by E7-T2 and E7-T4.
...
| AC-203 remote acceptance then crash before the local receipt commit enters unknown, reconciles by lookup, and never creates a second task | `TestG2AC203`: the lease is held while acceptance exists only remotely; lease expiry recovers submitting to unknown, the lookup proves acceptance, and the fake sink records zero additional submissions. (Correction per the 2026-08-22 review: the current test closes the store in-process and never invokes the fake sink, so the zero-additional-submissions assertion is hollow; the production `Recover` wiring and a real process-death test are restored by E7-T2.) |
...
| AC-207 interrupted migration leaves the database valid at the previous version; a restart reaches the new version, never partially assumed | `TestG2AC207`: the harness dies before applying migrations (the empty database is the valid previous version; the ledger exists only after the first migration, so the pre-ledger version read is skipped); a restart migrates to the newest version and passes the integrity check. (Correction per the 2026-08-22 review: the test currently skips entirely because the pre-ledger `SchemaVersion` read errors before any assertion runs, so this criterion has no executed evidence; interruption between and inside migration units is restored by E7-T4.) |
