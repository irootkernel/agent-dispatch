...
upgraded, backed up, and removed. JJUKKUMI is a set of one-shot CLI
...
reconciliation, and every jjukkumi invocation opens SQLite, does its
...
# dist/jjukkumi-v0.1.0-darwin-arm64
# dist/jjukkumi-v0.1.0-linux-amd64
# dist/SHA256SUMS
...
metadata). Copy the `jjukkumi` binary for your platform onto the PATH
(for example `/usr/local/bin/jjukkumi`).

## 2. Platform configuration and state paths

| Platform | Default config | Default state directory |
|---|---|---|
| macOS | `~/.config/jjukkumi/config.yaml` | `~/Library/Application Support/JJUKKUMI` |
| Linux | `$XDG_CONFIG_HOME/jjukkumi/config.yaml` (else `~/.config/jjukkumi/config.yaml`) | `$XDG_STATE_HOME/jjukkumi` (else `~/.local/state/jjukkumi`) |

Precedence everywhere: an explicit `--config` path, then the
`JJUKKUMI_CONFIG` environment variable, then the platform default; the
state directory comes from `instance.state_dir` in the configuration,
then `JJUKKUMI_STATE_DIR`, then the platform default. The state
directory must be a local filesystem outside the watched vault
(configuration-spec §3; `jjukkumi init` and every store open enforce
it, and `doctor` reports violations).

## 3. First-use installation on a clean host (macOS scenario)

```sh
jjukkumi init                              # writes the disabled example config,
                                           # creates the owner-only state directory
jjukkumi config validate --probe-targets   # configuration and target gates
jjukkumi watchman install --route wiki-maintenance
jjukkumi route enable --route wiki-maintenance \
    --acknowledge-production-gate <computed-route-revision> --yes
# install the daily schedule (section 4), then:
jjukkumi doctor                            # clean findings expected
```
...
JJUKKUMI daemon. Verified examples live in
`docs/examples/scripts/`:

- **macOS (launchd)**: `jjukkumi-reconcile.launchd.plist.example` —
  install into `~/Library/LaunchAgents/` and `launchctl load` it; it
  runs `jjukkumi reconcile --route <id> --reason scheduled --output
  json` daily at the configured time.
- **Linux (systemd --user)**: `jjukkumi-reconcile.service.example` and
  `jjukkumi-reconcile.timer.example` — install into
  `~/.config/systemd/user/`, `systemctl --user daemon-reload`, and
  `systemctl --user enable --now jjukkumi-reconcile.timer` (`daily`,
  persistent).
...
idempotently inspectable: `jjukkumi status` reports
`last_reconciled_at`, and `jjukkumi doctor` flags
...
2. Verify the new binary's version and schema range:
   `jjukkumi version --output json` (the schema range must cover the
   database's migration version).
3. Run `jjukkumi doctor` with the new binary without submitting work.
...
5. Verify integrity: `jjukkumi maintenance integrity --full`.
6. Re-inspect the trigger (`jjukkumi watchman status --route <id>` —
   the managed command embeds the binary path) and the schedule units.
7. Resume the route and run one reconciliation:
   `jjukkumi reconcile --route <id> --reason manual`.

## 6. Backup

```sh
jjukkumi maintenance backup --output /secure/path/state-$(date -u +%Y%m%d).db
```
