...
| AC-505 Linux CI suite pass | GitHub Actions CI was removed on 2026-08-22 (GitHub Actions is not used). The Linux leg of `make verify` (SCP-008) must be run manually on a supported Linux host; no such run is recorded yet, so this criterion currently has no CI evidence. |
