...
Daily reconciliation is scheduled externally through managed `launchd` on
macOS or managed systemd user units on Linux (D-029: supported platforms are
`darwin/arm64`, `linux/amd64`, and `linux/arm64`; E19-T8/T9). The schedule invokes
the CLI and does not require an Agent Dispatch daemon.
